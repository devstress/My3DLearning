...
- **NATS JetStream and Pulsar (`BrokerTransactionalClient` with publish-then-confirm):** For brokers without native transaction support, the `BrokerTransactionalClient` tracks every published message within the transaction scope. On success, the transaction commits normally. On failure or timeout, it publishes compensating tombstone messages to dedicated compensation topics (`{topic}.dlq`) for each already-published message, providing at-least-once semantics with compensation-based rollback. These compensation messages carry `compensated-message-id` and `compensation-reason` metadata so downstream consumers can distinguish rollback events from processing failures.

**CDC and Event Publishing:** The platform's `EventSourcing` module provides a separate but complementary capability: an append-only `IEventStore` with optimistic concurrency, forward/backward stream reads, and an `EventProjectionEngine` that rebuilds read-model state from event streams with configurable snapshot intervals. While the `ITransactionalClient` handles the *outbox* side (atomic write + publish), the EventSourcing module handles the *CDC* side — business events are appended atomically to the event store, and projections rebuild downstream state by replaying the event stream in order, similar to how CDC log tailing captures database changes and forwards them to consumers. Together, they form the two halves of the Transactional Outbox + CDC pattern: atomic writes (outbox) and reliable event propagation (projections).
...
These eleven pillars are not aspirational — each is implemented, tested, and operational. The platform's 2,239 automated tests (unit, integration, contract, workflow, browser, and load) verify that every pillar functions correctly in isolation and in composition.
...
- **Production maturity.** The platform is well-architected and covered by 2,239 automated tests across unit, integration, contract, workflow, browser, and load test suites. However, it is still early stage and needs additional validation to improve.
...
├── tests/
│   ├── UnitTests/                   # Fast, isolated unit tests (1,540 tests)
