...
The platform uses .NET 10, .NET Aspire, a configurable message broker layer, Temporal.io, CassandraDB, OpenTelemetry, and Ollama.  
...
- **Configurable message broker layer** — The platform uses the right messaging tool for each job:
  ...
  - **Configurable queue broker (e.g., RabbitMQ)** for task-oriented message delivery where queue semantics, lower HOL risk, or different consumption guarantees are needed. RabbitMQ provides independent per-recipient queues so saga processing of recipient B continues even when recipient A is down. Per-tenant topics at scale (e.g., 1 million tenants) are prohibitively expensive with Kafka; a queue broker with lightweight queue creation avoids this cost.
...
| 005 | Configurable message broker ingestion | Implement broker abstraction with Kafka and RabbitMQ providers for message ingestion | not-started |
